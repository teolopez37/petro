(function() {
	...
	var viewModel = {
		...
		searchByStateAndCity : function() {
			showMsgLoading();
			viewModel.stations([]);
			dataRequests.getCityStations({
				stateId : viewModel.selectedState(),
				cityId : viewModel.selectedCity()
					...
					function(response) {
						...
						$('.station2').css('width', ($(window).width() - 100) + 'px');
		...
		loadMap : function(pos) {
			...
			if (appData.selectedState != -1 && appData.selectedCity != -1) {
				viewModel.selectedState(appData.selectedState);
				viewModel.selectedCity(appData.selectedCity);
				appData.selectedState = -1;
				appData.selectedCity = -1;
				viewModel.searchByStateAndCity();
			}
